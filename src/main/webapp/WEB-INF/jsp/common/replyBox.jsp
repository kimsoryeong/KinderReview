...
<div class="bg-white rounded-2xl mt-3">
    ...
    <form action="/usr/reply/doWrite" method="post" class="space-y-4 flex flex-col" id="mainReplyForm">
        ...
        <div class="flex flex-col">
            <textarea id="content" name="content" rows="2" maxlength="500" 
                class="border border-gray-300 rounded-sm p-3 mt-2 text-gray-700 w-full"
  ...
  <strong>
    <c:choose>
      <c:when test="${reply.authLevel == 2}">
        ${reply.institutionName}
      </c:when>
      <c:otherwise>
        ${reply.nickname}
      </c:otherwise>
    ...
    ${fn:substring(reply.regDate, 2, 16)}
                            ...
                            <div class="flex items-center gap-2">
                                <c:if test="${reply.memberId == loginedMember.id}">
                                    <button type="button" class="text-gray-500 hover:text-blue-500 text-xs ml-2 edit-reply-btn"
                                ...
                                <button type="button" class="text-gray-500 hover:text-orange-500 text-xs ml-2 reply-toggle-btn"
                        ...
                        <form action="/usr/reply/doWrite" method="post" class="reply-form mt-2 ml-4 w-full hidden flex flex-col gap-2"
                            id="reply-form-${reply.id}">
                            <input type="hidden" name="relTypeCode" value="${relTypeCode}" />
                            <input type="hidden" name="relId" value="${relId}" />
                            <input type="hidden" name="parentId" value="${reply.id}" />
                            <input type="hidden" name="isSecret" value="false" class="isSecretInput" />
                            <textarea name="content" rows="1" class="border rounded-lg p-2 mt-1 text-gray-700 w-full" placeholder="답글을 입력하세요" required></textarea>
                        ...
                        <c:forEach var="child" items="${replies}">
                            <c:if test="${child.parentId == reply.id}">
                                <div class="ml-6 mt-2 pb-2 pl-4" id="${child.id}">
                                    <div class="flex items-center gap-2">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll('form').forEach(function(form) {
        const secretBtn = form.querySelector('.secretToggleBtn');
        const secretIcon = secretBtn && secretBtn.querySelector('i');
        const isSecretInput = form.querySelector('.isSecretInput');
