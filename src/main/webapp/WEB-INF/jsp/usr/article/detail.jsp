      ...
      <div class="flex justify-between mt-6">
        ...
        <c:if test="${article.getMemberId() == req.getLoginedMember().getId()}">
          <div class="flex gap-2">
            <a class="btn btn-neutral btn-outline" href="modify?id=${article.getId()}">수정</a>
            <a class="btn btn-neutral btn-outline" href="delete?id=${article.getId()}&boardId=${article.getBoardId()}" onclick="return confirm('정말로 삭제하시겠습니까?');">삭제</a>
          </div>
        </c:if>
      </div>
    </div>
